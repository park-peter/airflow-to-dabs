...
When the user provides an Airflow DAG file or asks about Airflow-to-Databricks migration, follow the workflow below to convert it into a complete Databricks Declarative Automation Bundles project (formerly Databricks Asset Bundles; DABs).
...
4. **Sensors**: Sensor tasks, their trigger conditions, whether their return values are consumed downstream, and their exact predicate/listing semantics (recursion, pagination, glob/suffix filters, sorting, timeout, and soft-fail behavior)
...
- `PythonOperator` / `@task` (TaskFlow) -> `notebook_task` / `spark_python_task` (extract callable into `src/<task_id>.py`). TaskFlow dataflow: a `@task` return -> `dbutils.jobs.taskValues.set(key="return_value", value=...)`, consumed via `{{tasks.<key>.values.return_value}}`; `multiple_outputs=True` (or a dict return) -> one task value per dict key. `.override(task_id=...)` sets the task key. Variant decorators: `@task.branch`/`@task.short_circuit` -> `condition_task`; `@task.bash` -> BashOperator rules; `@task.virtualenv`/`@task.external_python` -> `notebook_task` + env note; `@task.sensor` -> Tier-3 trigger only if a root sensor with unused `PokeReturnValue`, else flag; `@task.run_if`/`@task.skip_if` -> `run_if` only for status-equivalent predicates, else `condition_task` or flag; `@setup`/`@teardown` -> flag because no native lifecycle. Ordinary tasks need explicit `depends_on`/`run_if`; Airflow schedules teardown only after setup succeeds, while an ordinary teardown failure affects the Lakeflow job result and Airflow excludes it from DAG-run status by default.
...
- `SQLExecuteQueryOperator` is connection-agnostic -> `sql_task` only when its resolved connection targets Databricks SQL; a remote federatable DB read-only SELECT -> Lakehouse Federation `sql_task` over a foreign catalog; remote DML/DDL -> connector notebook or migrate target; unresolved connection -> flag (do not assume `sql_task`)
- Provider-specific `PostgresOperator` / `MySqlOperator` instances bind to their PostgreSQL/MySQL hooks, so the operator fixes the remote engine. Route read-only SELECTs through federation or a connector notebook, remote DML/DDL through a connector/API notebook or target migration, and recurring source->Delta ingestion through Lakeflow Connect when eligible. Never reinterpret them as Databricks SQL tasks from `conn_id` metadata
- **Snowflake operators** (no managed connector — route by intent): read-only SQL -> federation over a Snowflake foreign catalog; recurring Snowflake->Delta -> query-based Lakeflow Connect via foreign catalog; DML/DDL/`COPY`/`CALL` -> connector notebook or rewrite for Delta. `SnowflakeOperator` removed v6.0 (use `SQLExecuteQueryOperator`), `S3ToSnowflakeOperator` removed v5.0 (use `CopyFromExternalStageToSnowflakeOperator`); recognize both `@task.snowpark` (DAG syntax) and `snowpark_task` (API). Federation toward Snowflake is read-only
- **SQL data-quality checks** (`SQLColumnCheckOperator`/`SQLTableCheckOperator`/`SQLValueCheckOperator`/`SQLThresholdCheckOperator`/`SQLIntervalCheckOperator`/`Snowflake*CheckOperator`) -> `sql_task` using `assert_true(...)` so a failed check fails the task; preserve tolerance, interval ratios, partition/WHERE, null handling, dynamic thresholds — flag if not expressible. `@task.sql` is NOT a check (it's generic `SQLExecuteQueryOperator`) -> route by connection + SQL intent, `assert_true()` only if it is actually an assertion. `GenericTransfer` -> route by (source, destination) per the classification step
...
- `BranchDateTimeOperator` -> evaluator `notebook_task` + `condition_task`; preserve `use_task_logical_date`, timezone, inclusive bounds, overnight time-only rollover, both branch lists, and skip-propagation caveats
- `BranchDayOfWeekOperator` -> evaluator `notebook_task` + `condition_task`; preserve logical-date/wall-clock selection, DAG timezone, weekday normalization, both branch lists, and skip-propagation caveats
...
- **Cloud & messaging families** (AWS Athena/EMR/Glue/Lambda/Redshift/SageMaker/SQS/SNS; GCP BigQuery/Dataproc/Dataflow/PubSub; Azure ADF/Synapse; HTTP/SFTP/Kafka/Trino/Presto/Oracle/MSSQL/JDBC/SparkSql) -> no 1:1 task; route by intent (source-aware classification): remote query -> federation for federatable sources only (Athena/Trino/Presto are NOT federatable -> JDBC/SDK); recurring source->Delta -> Lakeflow Connect; remote compute Databricks replaces (EMR/Dataproc/SparkSql) -> migrate to notebook/`sql_task`/pipeline; retained remote orchestration (Lambda/Batch/Glue/SageMaker/ADF/Synapse/Dataflow) -> `notebook_task` driving the cloud SDK; Kafka consume->Delta -> Lakeflow Connect managed Kafka connector (continuous) or Structured Streaming; messaging side-effects (SNS/SQS/Kafka publish, Slack/PagerDuty) -> SDK/webhook `notebook_task`. State an import path as exact only if verified against provider docs, else describe by pattern + "confirm the module path"
...
- `S3KeySensor` / `HdfsSensor` / `FileSensor` -> job `trigger.file_arrival`; set `queue.enabled: true`, make trigger and ingestion discovery recurse over the same root, preserve the source glob/suffix predicate, and require an initial manual bootstrap run for existing files
...
2. **Tier 2 (semantic)**: Reason about the operator's intent.
   ...
   - `BranchDateTimeOperator` / `BranchDayOfWeekOperator`: evaluator notebook + `condition_task`; preserve logical-time selection, timezone/range/weekday semantics, branch lists, and skip behavior.
...
- **Execution date (`{{ ds }}`/`execution_date`) has no single Databricks equivalent** — classify per DAG, ask the user when ambiguous: wall-clock/"today's data" -> a `run_date` job parameter defaulting `{{job.start_time.iso_date}}`; logical/partition date (a `WHERE date = '{{ ds }}'`, a `.../{{ ds }}/...` path, an incremental cursor) on a **cron/scheduled** job -> default `{{job.trigger.time.iso_date}}` (the scheduled trigger time; `start_time` drifts with queue delay/retries). On an **event-triggered** job (`file_arrival`/`table_update`/`continuous`) `{{job.trigger.time}}` is unreliable — derive the date from the event (e.g. the arriving file path / `{{job.trigger.file_arrival.location}}`) or approximate with `start_time`, and flag. Note: an Airflow 2 scheduled `logical_date` is the START of the data interval (usually one period behind fire time), while `{{job.trigger.time}}` and Airflow 3's `CronTriggerTimetable` are the fire time — offset in code if the DAG relied on the interval-start convention, flag when unsure. ALWAYS map `{{ ds }}` to a named `run_date` job parameter, never a hardcoded date or inline `{{job.start_time...}}`, so native [Databricks backfill](https://docs.databricks.com/aws/en/jobs/backfill-jobs) can override it with `{{backfill.iso_date}}` (start of each replayed window) per run. `catchup=True` -> use native backfill (record in MIGRATION_NOTES that a backfill overrides `run_date` with `{{backfill.iso_date}}`); `catchup=False` -> no backfill. Backfills always run the whole job; pipeline tasks are NOT parameterized (can't carry a backfill date) -> flag a pipeline-only workload
- Map a static positive `dagrun_timeout` to Job `timeout_seconds`. Treat explicit disabled policy (`depends_on_past=False`, email flags `False`, `sla_miss_callback=None`, `max_consecutive_failed_dag_runs=0`, empty `env`) as no-ops. Flag active cross-run dependencies, retry email, arbitrary SLA callbacks, automatic pause after repeated failures, dynamic timeouts, and non-empty task environments; never substitute concurrency controls for prior-run or failure-history semantics.
- Airflow 3 `Asset`/`Dataset` scheduling -> `trigger.table_update` only when the asset resolves to a UC table via explicit `extra={"databricks_table": "catalog.schema.table"}`, a user-supplied mapping, or the skill-local `x-databricks-table:` URI scheme — else flag (an `Asset` URI is an arbitrary string; never infer a table from it). `schedule=[a, b]` (list) -> `ALL_UPDATED`; `a | b` -> `ANY_UPDATED`; `a & b` -> `ALL_UPDATED`. `AssetOrTimeSchedule` / `DatasetOrTimeSchedule` (time+asset) -> flag and emit a manual job with neither arm until the user chooses time, asset, or split jobs
- **`trigger_rule` -> `run_if`.** Lakeflow has exactly six values: ALL_SUCCESS, ALL_DONE, NONE_FAILED, AT_LEAST_ONE_SUCCESS, ALL_FAILED, AT_LEAST_ONE_FAILED. Exact: `all_success` -> ALL_SUCCESS (omit, it's the default), `all_done` -> ALL_DONE, `all_failed` -> ALL_FAILED, `one_success` -> AT_LEAST_ONE_SUCCESS, `one_failed` -> AT_LEAST_ONE_FAILED. Approximate (map + record the delta in MIGRATION_NOTES): `none_failed` and `none_failed_or_skipped` -> NONE_FAILED; `none_failed_min_one_success` -> NONE_FAILED, which drops the "at least one succeeded" clause so the task also runs when every upstream skipped — do NOT use AT_LEAST_ONE_SUCCESS, which would let it run while another upstream failed. No faithful mapping -> `condition_task` or flag: `always`/`dummy` (Airflow runs even when upstreams never ran; ALL_DONE still waits for a terminal state), `none_skipped`, `all_skipped` (defaulting inverts it to run only when upstreams succeeded), `one_done`, setup/teardown rules. An unrecognized or dynamically-computed rule must be flagged, never defaulted to ALL_SUCCESS
...
- Scheduling/backfill defaults changed: `schedule` defaults to `None` (unset = manual-trigger only, emit no DABs schedule/trigger); `catchup` defaults to `False` (only treat backfill as intended when `catchup=True` is explicit; note it in MIGRATION_NOTES); a raw-cron `schedule` uses `CronTriggerTimetable` (`logical_date` = fire time, no data interval) — confirm the intended window for a date-sensitive DAG before mapping `{{ ds }}`, else flag
- `fail_stop` DAG arg was renamed `fail_fast` (stop the run on first task failure); record the intent in MIGRATION_NOTES — a Lakeflow job has no single equivalent switch. `Param` is importable as `from airflow.sdk import Param` (params -> job parameters)
...
2. **`resources/<dag_id>_job.yml`**: One per DAG with schedule/trigger, Job `timeout_seconds` from static `dagrun_timeout`, email_notifications, parameters, job_clusters, tasks with `depends_on`. Cross-DAG `TriggerDagRunOperator` -> `${resources.jobs.<target>.id}`.
...
5. **`MIGRATION_NOTES.md`**: Tier 4 items, XCom patterns, Connections needing secrets, Variables needing parameters, settings without DABs equivalents, cross-DAG dependency map, every **collapsed retry envelope** (old and new retry boundaries plus repeated-side-effect risk), and setup/teardown lifecycle loss (Airflow schedules teardown only after setup succeeds; an ordinary teardown failure affects the Lakeflow job result while Airflow excludes it from DAG-run status by default). Factory mode: selector semantics, serverless-only note (classic via `job_cluster_key` in `DbtTaskOptions`), the measured task count and 1,000-task per-job limit (whether `BUNDLE_TESTS` was enabled + its retry-granularity tradeoff; over-limit-even-bundled options), profiles values to fill, `make setup && make manifest` prerequisite.
