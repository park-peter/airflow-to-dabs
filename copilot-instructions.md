...
- **SQL data-quality checks** (`SQLColumnCheckOperator`/`SQLTableCheckOperator`/`SQLValueCheckOperator`/`SQLThresholdCheckOperator`/`SQLIntervalCheckOperator`/`Snowflake*CheckOperator`/`@task.sql`) -> `sql_task` using `assert_true(...)` so a failed check fails the task; preserve tolerance, interval ratios, partition/WHERE, null handling, dynamic thresholds — flag if not expressible. `GenericTransfer` -> route by (source, destination) per the classification step
