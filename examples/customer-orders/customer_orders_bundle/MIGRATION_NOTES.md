...
| File discovery scope | **Broadened.** The source sensor was scoped to one partition per run (`orders/{{ ds }}/*.json`); the trigger and Auto Loader both scan all of `${var.landing_path}` recursively. `pathGlobFilter="*.json"` matches the file basename only, so it restores the JSON suffix contract but not the `orders/<date>/` directory scope. Every JSON under the landing path is eligible, including late-arriving files from earlier dates. `_ingest_run_date` stamps the run's `run_date` onto whatever the stream picks up, so it labels the ingest run and is not a partition filter — filter on `order_date` downstream when per-partition semantics matter. |
