...
- **Generated files:** `dbt_serverless_env.yaml` is rewritten idempotently at validate/deploy time, pinning the exact `dbt-databricks` and `dbt-core` versions installed in the bundle venv (not the databricks-dbt-factory version); change the pins in `pyproject.toml` and re-run `make setup`, don't edit the file. `src/run_dbt_command.py` is owned by the bundle (extended from the 0.2.1 packaged runner with `dbt_vars`/`dbt_target` support) — re-diff it against the packaged runner when bumping the pin.
