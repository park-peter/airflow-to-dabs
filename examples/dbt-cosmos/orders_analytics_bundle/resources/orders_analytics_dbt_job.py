...
Requires the databricks-bundles and databricks-dbt-factory pins in pyproject.toml and a
Databricks CLI that supports PyDABs. databricks-dbt-factory addresses each node with an
intersected selector it validates against dbt's own grammar, and emits one task per
dbt test (including unit tests); this glue prunes and bounds that output.
"""
...
# task (one dbt test with a --select term per test, at --indirect-selection empty) instead of one
...
def _write_serverless_environment_file() -> None:
    # dbt-core is pinned alongside dbt-databricks, which alone allows a dbt-core range:
    # the runner injects a parse cache built by the local dbt-core, so the serverless
    # runtime must resolve the identical version.
    spec = {
...
def _assert_unique_task_keys(tasks: list[dict]) -> None:
    """Defense-in-depth: databricks-dbt-factory derives readable task keys
    ...
    if duplicates:
        raise RuntimeError(
            ...
            + ", ".join(duplicates)
            + ". This should not happen with databricks-dbt-factory; rename the "
