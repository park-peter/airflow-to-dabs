...
# dbt's own selector matcher, imported from the dbt-core installed in the bundle venv so
# validation uses runtime dbt's semantics. Its signature gained an is_versioned parameter,
# so the arity decides which call form _call_is_selected_node uses.
