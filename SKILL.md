...
- `assets/templates/dbt-run-command.py.tmpl`: owned runner notebook (packaged-runner base + `dbt_vars` and per-target parse cache)
- `assets/templates/dbt-tests.py.tmpl`: regression tests for the generated glue (selector shape, `--vars` guard, task-key and task-limit guards, pruning)
