...
6. **Flags**: Note any custom operators (subclasses of BaseOperator), XCom usage (`xcom_push`/`xcom_pull`), Airflow Variables, Airflow Connections, dynamic task mapping (`.expand()`/`.expand_kwargs()`), **deferrable operators/sensors** (any Airflow version, 2.2+ — `deferrable=True`/`*DeferrableOperator`/`mode="reschedule"`; ignore the deferrability and map the underlying operation, no polling — see the deferrable note in `references/operator-mapping.md`), and custom timetable/dataset schedules. **Airflow 3-specific:** `Asset` scheduling (`schedule=[Asset(...)]`, boolean asset expressions, `AssetOrTimeSchedule`), native async `@task` (3.2.0 `async def`; rewrite hooks to native async clients), and resumable external jobs (`ResumableJobMixin`, 3.3.0; preserve reattachment or flag) — see `references/airflow3-migration.md`.
