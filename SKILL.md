...
3. **Tier 3 (sensors)**: Convert to job-level triggers. Read `references/schedule-trigger-mapping.md` in this skill's directory.
   ...
   - `BashSensor` / `PythonSensor`: convert to a supported job trigger only when the complete root predicate is provably equivalent and its output is unused; otherwise retain a polling notebook with timeout, soft-fail, return-value, and side-effect semantics documented.
