...
6. **Flags**: Note any custom operators (subclasses of BaseOperator), XCom usage (`xcom_push`/`xcom_pull`), Airflow Variables, Airflow Connections, dynamic task mapping (`.expand()`/`.expand_kwargs()`), **Airflow 3 `Asset` scheduling** (`schedule=[Asset(...)]`, boolean asset expressions, `AssetOrTimeSchedule`), **deferrable operators** (ignore — map the underlying operation, no polling), **native async `@task`** (Airflow 3.2.0 `async def`; rewrite hooks to native async clients), and **resumable external jobs** (`ResumableJobMixin`, Airflow 3.3.0; preserve reattachment or flag), and custom timetable/dataset schedules. See `references/airflow3-migration.md`.
